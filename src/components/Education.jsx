...
function JourneyColumn({ title, icon: Icon, items, visible, accent }) {
  return (
    <div>
      ...
      <div className="relative space-y-5 pl-5 before:absolute before:bottom-6 before:left-[5px] before:top-6 before:w-px before:bg-gradient-to-b before:from-white/20 before:to-transparent">
        {items.map((item, index) => {
          ...
          return (
            <motion.article key={item.id} initial={{ opacity: 0, x: accent === 'cyan' ? 24 : -24 }} animate={visible ? { opacity: 1, x: 0 } : {}} transition={{ duration: .6, delay: .2 + index * .12 }}
              className="glass-panel group relative rounded-[1.75rem] p-6 sm:p-7">
              <span className={`absolute -left-[1.25rem] top-9 h-2.5 w-2.5 rounded-full ring-4 ring-[#09090d] ${accent === 'cyan' ? 'bg-cyan-300 shadow-[0_0_15px_rgba(103,232,249,.7)]' : 'bg-primary-400 shadow-[0_0_15px_rgba(139,92,246,.7)]'}`} />
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <h4 className="font-display text-xl font-semibold tracking-tight text-white transition-colors group-hover:text-primary-200">{item.degree || item.role}</h4>
              ...
              <p className="mt-5 text-sm leading-7 text-zinc-500">{item.description}</p>
